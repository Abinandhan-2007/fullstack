...
export default function AttendanceMapping({ handleLogout, apiUrl }) {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        ...
        if (staffRes?.ok) setDbStaff(await staffRes.json());
  ...
  const handleAddMapping = async (e) => {
    ...
    const newMapping = {
      ...
      faculty: selectedStaffId,
      venue: selectedVenue,
      sessionType: selectedVenue.toLowerCase().includes('lab') ? 'Laboratory' : 'Theory',
      batchRange: (rollFrom && rollTo) ? `${rollFrom} to ${rollTo}` : 'All Students',
      seatAllocation: seatLayout 
    };
  ...
  const renderMappingStudio = () => (
    <div className="animate-in fade-in duration-200 max-w-7xl mx-auto space-y-6">
      ...
      <form onSubmit={handleAddMapping} className="flex flex-wrap gap-6 items-end">
          ...
          <div className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)]">
          ...
          <div className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)] bg-slate-50 p-3 rounded-xl border border-slate-100 h-[74px] flex flex-col justify-end relative">
          ...
          <div className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)]">
            ...
            <select className="w-full bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl px-3 py-3 text-sm font-bold outline-none focus:border-emerald-500 transition duration-150" value={seatingStrategy} onChange={(e) => setSeatingStrategy(e.target.value)}>
              ...
              <option value="exam">Exam Mode (Alternate Seats)</option>
            </select>
          </div>

          <div className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)] h-[46px] relative">
            <button type="submit" className={`w-full h-full rounded-xl font-bold text-white transition duration-150 shadow-md flex items-center justify-center ${!selectedSubject || !selectedStaffId || !selectedVenue || !selectedTime ? 'bg-slate-300 cursor-not-allowed' : 'bg-[#2563EB] hover:bg-blue-700 active:scale-[0.99] shadow-[#2563EB]/20'}`}>
              Confirm Mapping
            </button>
          </div>
        </form>
  ...
  if (selectedSessionForSeats && selectedSessionForSeats.seatAllocation) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <main className="flex-1 p-6 md:p-10 relative">
        {activeMenu === 'Workspace' ? renderWorkspace() : 
         ...
         <div className="text-center py-20"><h2 className="text-2xl font-bold text-slate-400">🚧 Coming Soon</h2></div>}
      </main>

      {/* --- ERROR/CONFLICT MODAL POPUP --- */}
      {conflictPopup && (
        <div className="fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl flex flex-col items-center text-center transform scale-100">
            ...
            <button onClick={() => setConflictPopup(null)} className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-3.5 rounded-xl transition duration-150 active:scale-95 shadow-md">
              Acknowledge & Edit
            </button>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-[95vw] lg:max-w-7xl h-[90vh] flex flex-col shadow-2xl overflow-hidden transform scale-100 transition-transform duration-200">
            <div className="p-6 border-b border-slate-100 bg-white">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-1"><span className="text-3xl">📍</span><h2 className="text-2xl font-black text-slate-800">{selectedSessionForSeats.venue} Map</h2></div>
                  <p className="text-sm font-bold text-slate-500 ml-10">{selectedSessionForSeats.subjectName} <span className="text-[#2563EB] mx-2">•</span> {selectedSessionForSeats.timeSlot}</p>
                </div>
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 ml-10">
                ...
                <div className="flex gap-5 items-center text-xs font-bold text-slate-500 bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-blue-600 shadow-sm border border-blue-700"></div> Found Match</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#E1EAF9] border border-[#C8D9F4]"></div> CSE</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#FEF4C1] border border-[#FDEB9E]"></div> ECE</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#FBE1EC] border border-[#F7CDE0]"></div> EEE</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#DBF6E4] border border-[#C3EED3]"></div> IT</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#F8FAFC] border border-slate-200"></div> Empty</span>
                </div>
              </div>
            </div>
                               ...
                               <td key={c} className="p-0 align-top relative">
                                 <div className={`flex flex-col items-center justify-center p-2.5 rounded-2xl border shadow-sm h-full min-h-[85px] transition-all duration-200 ${colorClasses}`}>
                                   {seatInfo && seatInfo.roll ? (
                                      <>
                                        <span className={`text-[10px] font-black mb-1 ${isMatch ? 'text-blue-200' : 'text-slate-500'}`}>{seatInfo.seat}</span>
                                        <span className="text-xs font-black uppercase tracking-wider mb-1.5">{seatInfo.dept}</span>
                                        <span className={`text-[9px] font-medium tracking-tight break-all text-center ${isMatch ? 'text-white' : 'text-slate-600'}`}>{seatInfo.roll}</span>
                                      </>
                                   ) : (
