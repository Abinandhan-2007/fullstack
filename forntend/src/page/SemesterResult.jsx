...
export default function SemesterResult({ semesterId, rollNo, onBack, userName }) {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        // Fetch detailed marks from Spring Boot
        ...
        if (data && data.length > 0) {
          ...
          // Calculate SGPA dynamically based on the subjects fetched
          let credits = 0;
          let earnedPoints = 0;
          data.forEach(sub => {
            credits += sub.credits;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500 ease-out pb-10 font-sans">
      ...
      <div className="flex items-center justify-between mb-8">
        <button 
          onClick={onBack}
          className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm"
        >
          ← Back to Ledger
        </button>
      ...
      {loading ? (
        <div className="flex justify-center py-32 text-slate-500 font-bold">
                  ...
                  {subjects.map((sub, index) => (
                    <tr key={index} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded border border-slate-200">{sub.courseCode}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-semibold text-slate-800">{sub.courseTitle}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{sub.courseType}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-sm font-medium text-slate-600">{sub.credits}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`text-lg font-black ${sub.grade === 'O' ? 'text-emerald-500' : 'text-blue-600'}`}>
                          {sub.grade}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className="text-sm font-bold text-slate-700">{sub.points}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
