...
import ParentFeePayment from './parent/ParentFeePayment';
...
export default function ParentPortal({ handleLogout, apiUrl, user, token, userName, loggedInEmail }) {
  ...
  const menuItems = [
    ...
    { name: 'Academic Marks', icon: '📊', category: 'Academic' },
    { name: 'Fee Payment', icon: '💳', category: 'Financial' },
    { name: 'Announcements', icon: '📢', category: 'Communication' },
    { name: 'Ward Profile', icon: '👤', category: 'Personal' },
  ];

  const categories = ['Overview', 'Academic', 'Financial', 'Communication', 'Personal'];

  const renderActiveModule = () => {
    switch (activeMenu) {
      case 'Dashboard': return <ParentDashboard {...moduleProps} />;
      case 'Attendance': return <ParentAttendance {...moduleProps} />;
      case 'Academic Marks': return <ParentMarks {...moduleProps} />;
      case 'Fee Payment': return <ParentFeePayment {...moduleProps} />;
      case 'Announcements': return <ParentAnnouncements {...moduleProps} />;
      case 'Ward Profile': return <ParentProfile {...moduleProps} />;
      ...
      <aside className={`fixed md:static inset-y-0 left-0 w-72 bg-white dark:bg-gray-900 border-r border-slate-200 dark:border-gray-800 z-30 transform transition-transform duration-300 ease-in-out flex flex-col shadow-xl md:shadow-none ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 flex items-center justify-between border-b border-slate-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-rose-500 to-rose-700 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-inner shadow-white/20">P</div>
            <div>
              <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Intuition</h1>
              <p className="text-[10px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mt-1">Parent Portal</p>
            </div>
          </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${
                        activeMenu === item.name
                          ? 'bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 shadow-sm'
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between shrink-0 shadow-sm relative z-10 transition-colors duration-200">
          ...
          <div className="flex items-center gap-6">
             ...
             <div className="hidden md:block text-right">
                <p className="text-sm font-bold text-slate-800 dark:text-white">{userName || 'Parent Account'}</p>
                <p className="text-[10px] uppercase font-bold text-rose-500 tracking-widest flex items-center justify-end gap-1"><span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span> GUARDIAN</p>
             </div>
             <div className="w-10 h-10 bg-rose-100 dark:bg-rose-900/30 rounded-full border border-rose-200 dark:border-rose-800 flex items-center justify-center text-rose-600 dark:text-rose-400 cursor-pointer shadow-sm">
                👤
             </div>
          </div>
        </header>
