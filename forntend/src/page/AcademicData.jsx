          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="border-b border-slate-200 bg-white">
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Subjects</th>
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Credits</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {academicInfo.semesterRecords && [...academicInfo.semesterRecords].reverse().map((sem, index) => {
                ...
                return (
                  <tr 
                    key={index} 
                    onClick={() => onViewSemester(sem.semesterName)}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center"><span className="text-sm text-slate-700 font-medium">{subjectsCount}</span></td>
                    <td className="px-6 py-4 text-center"><span className="text-sm text-emerald-600 font-semibold">{clearedCount}</span></td>
                    <td className="px-6 py-4 text-center">
                      <span className={`text-sm font-semibold ${arrearsCount > 0 ? 'text-red-600' : 'text-slate-400'}`}>{arrearsCount}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm text-slate-800 font-semibold bg-slate-100 px-2 py-1 rounded border border-slate-200">{creditsEarned}</span>
                    </td>
                    <td className="px-6 py-4 text-center"><span className="text-sm font-bold text-blue-600">{sem.sgpa.toFixed(2)}</span></td>
                    <td className="px-6 py-4 text-right">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-bold tracking-wider uppercase ${
                        statusStr === 'Verified' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200'
                      }`}>{statusStr}</span>
                    </td>
                  </tr>
                );
