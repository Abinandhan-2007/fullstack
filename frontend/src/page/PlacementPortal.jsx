...
import PlacementCompanies from './placement/PlacementCompanies';
import PlacementDrives from './placement/PlacementDrives';
import PlacementStudentTrack from './placement/PlacementStudentTrack';
import PlacementOffers from './placement/PlacementOffers';
import PlacementTraining from './placement/PlacementTraining';
import PlacementAnalytics from './placement/PlacementAnalytics';
import PlacementSettings from './placement/PlacementSettings';

export default function PlacementPortal({ handleLogout, apiUrl, user, token, userName, loggedInEmail }) {
  ...
  const menuItems = [
    ...
    { name: 'Corporate Partners', icon: '🏢', category: 'Partnerships' },
    { name: 'Placement Drives', icon: '🚀', category: 'Operations' },
    { name: 'Student Eligibility', icon: '📋', category: 'Data' },
    { name: 'Offer Letters', icon: '📑', category: 'Data' },
    { name: 'Training Programs', icon: '🎓', category: 'Skill Dev' },
    { name: 'Drive Analytics', icon: '📊', category: 'Insights' },
    { name: 'Portal Settings', icon: '⚙️', category: 'General' },
  ];

  const categories = ['Main', 'Partnerships', 'Operations', 'Data', 'Skill Dev', 'Insights', 'General'];

  const renderActiveModule = () => {
    switch (activeMenu) {
      case 'Dashboard': return <PlacementDashboard {...moduleProps} />;
      case 'Corporate Partners': return <PlacementCompanies {...moduleProps} />;
      case 'Placement Drives': return <PlacementDrives {...moduleProps} />;
      case 'Student Eligibility': return <PlacementStudentTrack {...moduleProps} />;
      case 'Offer Letters': return <PlacementOffers {...moduleProps} />;
      case 'Training Programs': return <PlacementTraining {...moduleProps} />;
      case 'Drive Analytics': return <PlacementAnalytics {...moduleProps} />;
      case 'Portal Settings': return <PlacementSettings {...moduleProps} />;
      ...
      <aside className={`fixed md:static inset-y-0 left-0 w-72 bg-white dark:bg-gray-900 border-r border-slate-200 dark:border-gray-800 z-30 transform transition-transform duration-300 ease-in-out flex flex-col shadow-xl md:shadow-none ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 flex items-center justify-between border-b border-slate-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-inner shadow-white/20">P</div>
            <div>
              <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Intuition</h1>
              <p className="text-[10px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mt-1">Placement Portal</p>
            </div>
          </div>
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between shrink-0 shadow-sm relative z-10 transition-colors duration-200">
          ...
          <div className="flex items-center gap-6">
             ...
             <div className="hidden md:block text-right">
                <p className="text-sm font-bold text-slate-800 dark:text-white">{userName || 'Placement Officer'}</p>
                <p className="text-[10px] uppercase font-bold text-emerald-500 tracking-widest flex items-center justify-end gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> OFFICE HEAD</p>
             </div>
